  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async {
        context
            .read<StoreBloc>()
            .add(LoadStoreTransactions(id: widget.storeId, typeIds: 1));
      },
                  ...
                  BlocBuilder<StoreBloc, StoreState>(
                    builder: (context, state) {
                      if (state is StoreTransactionLoading) {
                        ...
                        var transactions = state.activityTransactions;
                        if (transactions == null) {
                          ...
                          return ListView.builder(
                            ...
                            itemCount: state.hasReachedMax
                                ? state.activityTransactions!.length
                                : state.activityTransactions!.length + 1,
                            itemBuilder: (context, index) {
                              if (index >= state.activityTransactions!.length) {
                                return Center(
                                ...
                                return TransactionCard(
                                  ...
                                  ffem: widget.ffem,
                                  transaction:
                                      state.activityTransactions![index],
                                );
                              }
                            },
                          );
